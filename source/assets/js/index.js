...
_.extend(app, {
	...
	checkConfig: function() {
		...
		// console.log("[checkConfig] app.data.version:" app.data.version);
		// console.log("[checkConfig] config.versions.compatibility:" config.versions.compatibility);
		// console.log("[checkConfig] config.versions.compatibility:" config.versions.compatibility);

		var versions = {
			current: app.data.version.split('.'),
			compatibility: config.versions.compatibility.split('.'),
			production: config.versions.compatibility.split('.')
		}
	...
	showConfigNotification: function(type) {
		...
		switch(type) {
			...
			case 'killSwitch':
				html = '<div class="text">' +
					'<div>HS Hackers is currently unavailable.</div>' +
			...
			case 'versionIncompatibility':
				...
				html = '<div class="text">' +
					'<div>A new version of the HS Hackers app is now available.</div>' +
	...
	resumeSession: function() {
		...
		var retry = function() {
			app.showNotification('Oops!', 'There was an error communicating with HS Hackers, please wait while we attempt to re-connect in 5 seconds.', false, function() {
...
app.on('init', function() {
	...
	// Show the loading view immeidately, which is a clone of the home view with the HS Hackers logo
